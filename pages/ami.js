...
function Tracker({ pseudo, onChangePseudo }) {
  ...
  const storageKey = `ami_progression_${pseudo}`;

  // Chargement progression locale
  useEffect(() => {
    try {
  ...
  // Chargement sections + polling 2 min
...
export default function Ami() {
  ...
  return (
    <>
      ...
      <main>
        {pseudo
          ? <Tracker pseudo={pseudo} onChangePseudo={handleChangePseudo} />
